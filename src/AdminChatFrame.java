...
public class AdminChatFrame extends JFrame {

    JTextArea chatArea;
    JTextField msgField;
    JComboBox<String> studentBox;

    public AdminChatFrame() {
        ...
        JLabel title = new JLabel("Admin Chat");
        ...
        studentBox.setPreferredSize(new Dimension(200, 30));
        selectorPanel.add(studentBox);
    ...
    void loadStudents() {
        try {
            ...
            ResultSet rs = st.executeQuery(
                    "SELECT DISTINCT sender FROM messages WHERE sender != 'admin'"
            );

            while (rs.next()) {
                studentBox.addItem(rs.getString("sender"));
            }
            con.close();
        } catch (Exception e) {
            e.printStackTrace();
